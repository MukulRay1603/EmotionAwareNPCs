            ...
            while self.running:
                ...
                if current_time - last_update_time >= UPDATE_INTERVAL:
                    ...
                    if results.multi_face_landmarks:
                        ...
                        current_emotion_data = emotion_data
                ...
                if current_emotion_data:
                    ...
                    display_flag = current_emotion_data['flag']
                    ...
                    # Feature values
                    y_pos = 130
                    cv2.putText(display_frame, f"Emotion: {current_emotion_data['emotion']}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                    y_pos += 30
                    cv2.putText(display_frame, 
                              f"Valence: {current_emotion_data['valence']:+.2f} | Arousal: {current_emotion_data['arousal']:+.2f}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                    y_pos += 30
                    spike_indicator = '🔥' if current_emotion_data['is_spike'] else ''
                    cv2.putText(display_frame, 
                              f"Delta E: {current_emotion_data['delta_E']:.3f} {spike_indicator}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                    y_pos += 30
                    fatigue_indicator = '💤' if current_emotion_data['is_fatigued'] else ''
                    cv2.putText(display_frame, 
                              f"EAR: {current_emotion_data['ear']:.3f} {fatigue_indicator}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                    y_pos += 30
                    cv2.putText(display_frame, 
                              f"Motion: {current_emotion_data['flow_magnitude']:.2f}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                    y_pos += 30
                    cv2.putText(display_frame, 
                              f"Confidence: {current_emotion_data['confidence']:.2f}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                    
                    # Scores
                    y_pos += 40
                    cv2.putText(display_frame, 
                              f"Stress: {current_emotion_data['scores']['stress']:.2f}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                    y_pos += 25
                    cv2.putText(display_frame, 
                              f"Rush: {current_emotion_data['scores']['rush']:.2f}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 2)
                    y_pos += 25
                    cv2.putText(display_frame, 
                              f"Fatigue: {current_emotion_data['scores']['fatigue']:.2f}", 
                              (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
                    
                    # Database size
                    cv2.putText(display_frame, f"DB Size: {len(self.emotion_database)}", 
                              (10, frame.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 
                              0.6, (255, 255, 255), 2)
                
                # Show frame
                cv2.imshow('Background Fusion Service - Live Display', display_frame)
